          ...
          <tbody>
            {sorted.map(({ player, activePos }) => {
              ...
              const dAvgR =
               ...
               const ppgV1s = s ? s.ppgV1.toFixed(2) : '—';
              const ppgV2s = s ? s.ppgV2.toFixed(2) : '—';
              const p90V1s = s ? s.p90V1.toFixed(2) : '—';
              const p90V2s = s ? s.p90V2.toFixed(2) : '—';
              ...
              return (
                <tr key={player.id} className={styles.shadowRow}>
                  ...
                  <td className={styles.shadowTdNum}>{s ? s.ptsV1.toFixed(2) : '—'}</td>
                  <td className={styles.shadowTdNum}>{s ? s.ptsV2.toFixed(2) : '—'}</td>
                  <td className={`${styles.shadowTdNum} ${dPts != null ? (dPts >= 0 ? styles.cellNumPos : styles.cellNumNeg) : ''}`}>
                    {dPts != null ? `${dPts >= 0 ? '+' : ''}${dPts.toFixed(2)}` : '—'}
                  </td>
