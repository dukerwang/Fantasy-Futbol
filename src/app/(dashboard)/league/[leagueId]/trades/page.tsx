...
export default async function TradesPage({ params }: Props) {
  ...
  if (allTeamIds.length > 0) {
    const { data: entries } = await admin
      .from('roster_entries')
      .select('team_id, on_trade_block, player:players(id, fpl_id, api_football_id, web_name, name, full_name, first_name, second_name, date_of_birth, nationality, pl_team, pl_team_id, primary_position, secondary_positions, market_value, market_value_updated_at, projected_points, photo_url, height_cm, fpl_status, fpl_news, total_points, form_rating, ppg, is_active, created_at, updated_at)')
  ...
  const { data: myRosterEntries } = await admin
    .from('roster_entries')
    .select('on_trade_block, player:players(id, fpl_id, api_football_id, web_name, name, full_name, first_name, second_name, date_of_birth, nationality, pl_team, pl_team_id, primary_position, secondary_positions, market_value, market_value_updated_at, projected_points, photo_url, height_cm, fpl_status, fpl_news, total_points, form_rating, ppg, is_active, created_at, updated_at)')
  ...
  if (allPlayerIds.size > 0) {
    const { data: players } = await admin
      .from('players')
      .select('id, fpl_id, api_football_id, web_name, name, full_name, first_name, second_name, date_of_birth, nationality, pl_team, pl_team_id, primary_position, secondary_positions, market_value, market_value_updated_at, projected_points, photo_url, height_cm, fpl_status, fpl_news, total_points, form_rating, ppg, is_active, created_at, updated_at')
