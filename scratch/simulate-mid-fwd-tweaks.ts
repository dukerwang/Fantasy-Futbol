...
const PROPOSED_WEIGHTS: Record<string, Record<string, number>> = {
  ...
  CM: { match_impact: 0.25, influence: 0.10, creativity: 0.15, threat: 0.05, defensive: 0.15, goal_involvement: 0.10, finishing: 0.00, save_score: 0.00 },
  AM: { match_impact: 0.15, influence: 0.10, creativity: 0.20, threat: 0.10, defensive: 0.00, goal_involvement: 0.15, finishing: 0.10, save_score: 0.00 },
  LW: { match_impact: 0.15, influence: 0.10, creativity: 0.15, threat: 0.15, defensive: 0.00, goal_involvement: 0.15, finishing: 0.10, save_score: 0.00 },
  RW: { match_impact: 0.15, influence: 0.10, creativity: 0.15, threat: 0.15, defensive: 0.00, goal_involvement: 0.15, finishing: 0.10, save_score: 0.00 },
};
...
function calculateSimulatedRating(
    ...
    const weights = useTweaks ? PROPOSED_WEIGHTS[position] : ORIGINAL_WEIGHTS[position];
...
(async () => {
  ...
  console.log("=== SIMULATING PROPOSED MID/FWD WEIGHTS ===");
  ...
  for (const item of playersToTest) {
    ...
    for (const r of statsRows) {
      ...
      const orig = calculateSimulatedRating(stats, p.primary_position, refStats, false);
      const prop = calculateSimulatedRating(stats, p.primary_position, refStats, true);
