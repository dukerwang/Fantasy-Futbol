...
function calculateRating(stats: any, position: string, refStats: any, primaryPosition: string) {
    ...
    // Option B OOP penalty in old deployed code:
...
(async () => {
  const p = { id: 'ad4ecab9-d1d8-46be-b6ef-7e0aee0efa14' }; // Szoboszlai
  ...
  for (const r of statsRows || []) {
    ...
    const rb = calculateRating(s, 'RB', refStats, 'AM').fantasyPoints;
    sumRb += rb;
  }

  console.log(`Played Games: ${playedCount}`);
  console.log(`PPG as RB under OLD deployed code: ${(sumRb / playedCount).toFixed(2)}`);
})();
