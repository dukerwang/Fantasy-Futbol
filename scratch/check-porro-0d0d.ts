...
// Medians/stddevs from commit 0d0d5730
...
function calculateRating0d0d(stats: any, position: string, primaryPosition: string) {
    ...
    // In commit 0d0d5730, defActionsRaw for fullbacks (LB/RB/LWB/RWB) was dc + recoveries * 0.5;
...
(async () => {
  ...
  for (const r of statsRows || []) {
    ...
    const rb = calculateRating0d0d(s, 'RB', 'RB').fantasyPoints;
    const rwb = calculateRating0d0d(s, 'RWB', 'RB').fantasyPoints;
  ...
  console.log(`PPG as RB under commit 0d0d : ${(sumRb / playedCount).toFixed(2)}`);
  console.log(`PPG as RWB under commit 0d0d: ${(sumRwb / playedCount).toFixed(2)}`);
})();
