...
const CONFIGS = {
  DM_PROPOSED: {
    ...
    flex: { flex: 0.25, components: ['match_impact', 'influence', 'defensive'] }
  },
  CM_PROPOSED: {
    weights: { match_impact: 0.25, influence: 0.05, creativity: 0.15, threat: 0.00, defensive: 0.20, goal_involvement: 0.10, finishing: 0.00, save_score: 0.00 },
    flex: { flex: 0.25, components: ['match_impact', 'creativity', 'influence'] }
  }
};
...
function calculateSimulatedRating(
    ...
    cfg: typeof CONFIGS.DM_PROPOSED
) {
    ...
    for (const key of Object.keys(cfg.weights)) {
        const weight = (cfg.weights as any)[key];
...
(async () => {
  ...
  console.log("=== COMPARING PLAYERS EVALUATED AS DM vs CM ===");
  ...
  for (const item of players) {
    ...
    let gp = 0;
    let ptsAsDM = 0, ratAsDM = 0;
    let ptsAsCM = 0, ratAsCM = 0;
    ...
    for (const r of statsRows) {
      ...
      const asDM = calculateSimulatedRating(stats, 'DM', refStats, CONFIGS.DM_PROPOSED);
      const asCM = calculateSimulatedRating(stats, 'CM', refStats, CONFIGS.CM_PROPOSED);

      ptsAsDM += asDM.fantasyPoints;
      ratAsDM += asDM.rating;

      ptsAsCM += asCM.fantasyPoints;
      ratAsCM += asCM.rating;
    }

    console.log(`Player: ${p.name} (Games Analyzed: ${gp})`);
    console.log(`  - AS DM (Proposed): Rating = ${(ratAsDM / gp).toFixed(2)}, PPG = ${(ptsAsDM / gp).toFixed(1)}`);
    console.log(`  - AS CM (Proposed): Rating = ${(ratAsCM / gp).toFixed(2)}, PPG = ${(ptsAsCM / gp).toFixed(1)}`);
    console.log("--------------------------------------------------");
  }
})();
