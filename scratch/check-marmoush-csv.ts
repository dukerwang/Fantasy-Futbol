...
(async () => {
  ...
  console.log("=== MARMOUSH DIAGNOSIS ===");
  ...
  for (const [element, rows] of playerRowsMap.entries()) {
    ...
    if (rawCsvName.toLowerCase().includes('marmoush') || normName.includes('marmoush')) {
      console.log(`Found direct CSV Name match: '${rawCsvName}' (element ${element}), Team: '${rows[0][colIdx('team')]}'`);
    }

    const matchedRealName = nameToRealName.get(normName);
    if (matchedRealName === 'Omar Marmoush' || matchedRealName?.includes('Marmoush')) {
      console.log(`Matched Real Name: '${matchedRealName}' (element ${element})`);
      console.log(`  - Raw CSV Name: '${rawCsvName}'`);
      console.log(`  - CSV Team: '${rows[0][colIdx('team')]}'`);
