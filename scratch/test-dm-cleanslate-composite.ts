...
(async () => {
  ...
  const penaltiesToTest = [0.00, 0.04, 0.06, 0.08, 0.10];

  console.log("\n=== SIMULATING VARIOUS COMPOSITE PENALTY COEFFICIENTS ===\n");
  ...
  for (const item of players) {
    ...
    console.log(`Player: ${p.name} (${pos}) | Team ID: ${p.pl_team_id}`);
    
    for (const coeff of penaltiesToTest) {
      let gp = 0;
      let totalPts = 0;
      let totalRat = 0;
      ...
      for (const r of statsRows) {
        ...
        const res = calculateSimulatedRating(enrichedStats, pos, refStats, coeff);
        totalPts += res.fantasyPoints;
        totalRat += res.rating;
      }

      console.log(`  - Coeff: ${coeff.toFixed(2)} | Rating = ${(totalRat / gp).toFixed(2)} | PPG = ${(totalPts / gp).toFixed(1)}`);
    }
    console.log("--------------------------------------------------");
  }
})();
