...
async function main() {
  ...
  // We want to test a grid of configurations and score them based on FPL alignment.
  // FPL alignment criteria:
  // 1. Raya PPG > Roefs PPG (Raya is 18 clean sheets / 0.50 conceded; Roefs is 10 clean sheets / 1.62 conceded).
  // 2. Raya is at the very top (excluding 1-3 game outliers like Benitez or Trafford).
  // 3. Donnarumma is in top 3/4 regular starters.
  // 4. Roefs PPG is reasonable (around 10.0 - 11.5 PPG).
  // 5. Mads Hermansen is at realistic standing (around 10.0 - 11.0 PPG).
  
  const csBonuses = [15, 18, 20];
  const saveFloors = [0.70, 0.75, 0.80];
  const gcPenalties = [1.5, 2.0, 2.5, 3.0, 3.5, 4.0];
  const defWeights = [0.25, 0.28, 0.30, 0.32];
  const saveWeights = [0.03, 0.05, 0.07, 0.10];
    ...
    for (const saveFloorOnCS of saveFloors) {
      for (const gcPenalty of gcPenalties) {
        for (const defWeight of defWeights) {
          ...
          const kelleher = starters.find(r => r.name.includes("Kelleher"));
          const pickford = starters.find(r => r.name.includes("Pickford"));
          
          if (!raya || !roefs || !donnarumma) continue;

          // Let's compute a score for how good this calibration is:
          // We want:
          // 1. Raya to be ahead of Roefs: raya.ppg > roefs.ppg
          // 2. Raya to be #1 among starters
          // 3. Donnarumma to be #2 or #3 among starters
          // 4. The overall scale should not be crazy (Raya PPG between 10.5 and 12.5)
          
          let score = 0;
          
          // Raya vs Roefs: Raya should ideally be higher because in FPL he is 4.25 vs 3.93.
          if (raya.ppg > roefs.ppg) {
            score += 100;
            // The margin by which Raya is ahead (cap it at 1.0)
            score += Math.min(1.0, raya.ppg - roefs.ppg) * 50;
          } else {
            // Penalize how far behind Raya is
            score -= (roefs.ppg - raya.ppg) * 100;
          }

          // Raya Rank among starters
          const rayaRank = starters.findIndex(r => r.name.includes("Raya")) + 1;
          if (rayaRank === 1) score += 150;
          else if (rayaRank === 2) score += 80;
          else if (rayaRank === 3) score += 40;
          else score -= rayaRank * 20;

          // Donnarumma Rank
          const donnaRank = starters.findIndex(r => r.name.includes("Donnarumma")) + 1;
          if (donnaRank <= 3) score += 80;
          else if (donnaRank === 4) score += 40;
          else score -= donnaRank * 10;

          // Reasonable overall scale (GKs shouldn't average > 13 PPG or < 8 PPG)
          if (raya.ppg >= 10.0 && raya.ppg <= 12.5) score += 50;
          else score -= Math.abs(raya.ppg - 11.25) * 50;

          candidates.push({
            config: { csBonus, saveFloorOnCS, defWeight, saveWeight, gcPenalty },
            score,
  ...
  console.log(`\n=== TOP 5 CALIBRATION CANDIDATES ===`);
  for (let i = 0; i < Math.min(5, candidates.length); i++) {
    const c = candidates[i];
    console.log(`\nRank ${i+1} | Score: ${c.score.toFixed(1)}`);
    console.log(`Config: csBonus=${c.config.csBonus}, saveFloorOnCS=${c.config.saveFloorOnCS}, defWeight=${c.config.defWeight.toFixed(2)}, saveWeight=${c.config.saveWeight.toFixed(2)}, gcPenalty=${c.config.gcPenalty}`);
