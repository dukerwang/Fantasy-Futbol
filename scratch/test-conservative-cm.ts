...
const ORIGINAL_WEIGHTS: Record<string, Record<string, number>> = {
  ...
  AM: { match_impact: 0.10, influence: 0.10, creativity: 0.25, threat: 0.15, defensive: 0.00, goal_involvement: 0.15, finishing: 0.00 }
};

const PROPOSED_CONSERVATIVE_WEIGHTS: Record<string, Record<string, number>> = {
  // Conservative CM Profile
  CM: { match_impact: 0.25, influence: 0.10, creativity: 0.15, threat: 0.05, defensive: 0.10, goal_involvement: 0.10, finishing: 0.00 },
  // Let's also include proposed AM weights
  AM: { match_impact: 0.15, influence: 0.10, creativity: 0.20, threat: 0.15, defensive: 0.00, goal_involvement: 0.15, finishing: 0.00 }
};
...
(async () => {
  ...
  const playersToTest = [
    ...
    { name: "Alexis Mac Allister", expectedPos: "CM" },
    { name: "Bernardo Silva", expectedPos: "CM" }
  ];

  console.log("=== SIMULATING CONSERVATIVE CM / AM UPGRADES ===");
  ...
  for (const item of playersToTest) {
    ...
    for (const r of statsRows) {
      ...
      const propPrim = calculateSimulatedRating(stats, pos, refStats, PROPOSED_CONSERVATIVE_WEIGHTS[pos] || PROPOSED_CONSERVATIVE_WEIGHTS.CM);
      ...
      const propCM = calculateSimulatedRating(stats, 'CM', refStats, PROPOSED_CONSERVATIVE_WEIGHTS.CM);
