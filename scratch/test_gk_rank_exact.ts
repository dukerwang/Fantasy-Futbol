...
async function main() {
  ...
  // Exact configuration to test (Rank 2 winner from grid search)
  const config = { csBonus: 16, saveFloorOnCS: 0.85, defWeight: 0.32, saveWeight: 0.03, gcPenalty: 4.0 };
  ...
  console.log(`\n=== CALIBRATED GK RANKS (csBonus=${config.csBonus}, saveFloor=${config.saveFloorOnCS}, defWeight=${config.defWeight}, saveWeight=${config.saveWeight}, gcPenalty=${config.gcPenalty}) ===`);
