...
// Define the two historical configs
const CONFIGS = [
  {
    name: 'Current V2 (Reverted State / No Minute Penalty / Defensive Focus)',
    weights: {
    ...
    hasMinutePenalty: false
  },
  {
    name: 'Older V2 (Pre-eaee1181 / With Minute Penalty / Balanced)',
    weights: {
...
function simulateMatchRating(
  ...
  // Calculate raw component scores using original formula
  ...
  components.influence = sigmoidNormalize(parseFloat((stats.fpl_influence ?? 0).toString()), ref.influence.median, ref.influence.stddev);
  components.creativity = sigmoidNormalize(parseFloat((stats.fpl_creativity ?? 0).toString()), ref.creativity.median, ref.creativity.stddev);
  components.threat = sigmoidNormalize(parseFloat((stats.fpl_threat ?? 0).toString()), ref.threat.median, ref.threat.stddev);
  ...
  if (config.hasMinutePenalty && stats.minutes_played < 60) {
    scoringRating = Math.max(1.0, scoringRating - (1 - stats.minutes_played / 60) * 1.5);
  }
  ...
  return {
    ...
    fantasyPoints: Math.max(0, Math.round(finalPoints * 10) / 10)
  };
}
