-- Scout's Fee was paying out whenever a manager was first to bid on a player,
-- even when the auction itself was opened by the SYSTEM (a new-arrival/
-- promoted-club sweep, a drop re-auction, a post-departure return auction,
-- season kickoff) rather than by a manager nominating a free agent. The
-- design intent (docs/superpowers/specs/2026-07-29-economy-rebalance-design.md:134)
-- is "the first manager to bid" as a proxy for "the manager who surfaced this
-- signing" — that proxy breaks once the system has already opened the
-- auction before any manager acted, since being first to bid there is luck
-- (or notification speed), not scouting.
--
-- Restricts the fee to auctions a manager genuinely opened: the anchor row
-- born lazily inside place_auction_bid_rpc when a manager nominates a free
-- agent with no pending claim at all yet (migration 125's "free agents are
-- born here"). Every other anchor-creating path is marked system_seeded so
-- the resolver can tell the two apart.
--
-- New column defaults to FALSE, so every anchor already in the database —
-- including the live Barcola arrival auction — is unaffected: Stamford &
-- Sons still collects the Scout's Fee when that one resolves. Only auctions
-- opened by the updated application code after this migration are marked
-- TRUE and lose eligibility.

ALTER TABLE public.waiver_claims
  ADD COLUMN IF NOT EXISTS system_seeded BOOLEAN NOT NULL DEFAULT FALSE;

COMMENT ON COLUMN public.waiver_claims.system_seeded IS
  'TRUE on an anchor row (team_id IS NULL) that the system opened — a new-arrival/promoted-club sweep, a drop re-auction, or a post-departure return auction — rather than a manager nominating a free agent. Gates Scout''s Fee eligibility in resolve_single_player_auction_rpc.';
