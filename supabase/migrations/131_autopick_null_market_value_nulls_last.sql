-- Postgres sorts NULL as the largest value by default. `ORDER BY
-- market_value DESC` therefore put unsynced players (market_value IS NULL)
-- at the FRONT of the candidate pool, and PERCENT_RANK() OVER (ORDER BY
-- market_value) handed them value_pct ~= 1.0 -- the algorithm was reading
-- "no market value on file" as "most valuable player available." Add
-- explicit NULLS LAST everywhere market_value is ordered so unsynced
-- players fall to the bottom instead of the top.
    ...
    IF v_best_candidate IS NULL THEN
      ...
      FOR v_player_rec IN
        ...
        SELECT
          ...
          PERCENT_RANK() OVER (ORDER BY c.market_value NULLS LAST) AS value_pct,
