-- ============================================================
-- Migration 051: Idempotency and Atomic Offseason/Transfer RPCs
-- ============================================================

-- 1. Add Unique Constraint to Tournaments to prevent duplicates
ALTER TABLE public.tournaments
  ADD CONSTRAINT unique_league_type_season UNIQUE (league_id, type, season);

-- 2. Make credit_faab_prize idempotent
...
AS $$
BEGIN
  -- Idempotency check: see if this specific prize payout has already been recorded
  IF EXISTS (
    SELECT 1 FROM public.transactions
    WHERE league_id = p_league_id
      AND team_id = p_team_id
      AND type = 'prize_payout'
      AND notes = p_prize_name
  ) THEN
...
-- 3. Create process_player_transfer_out_rpc for atomic and idempotent transfer payouts
...
BEGIN
  ...
  -- 3. Loop over roster entries for active leagues and lock affected teams
  FOR r IN
...
-- 4. Revoke EXECUTE permission from PUBLIC, anon, and authenticated roles
