-- Gaffa — Migration 128: pull back Draft Rank's performance weight
--
-- Mirrors src/lib/draft/autoPickEngine.ts's MAX_PERFORMANCE_WEIGHT change
-- (0.45 -> 0.35). Real feedback drafting against the live Rank column: a
-- standout-PPG season for a lower-profile player (e.g. a DM) could outrank
-- a much higher-value, higher-pedigree star on a comparatively small value
-- gap. Season stats now cap at 35% of a full-sample player's score instead
-- of 45%; market value gets the rest. Only the one constant changes — same
-- body as migration 125 otherwise.
