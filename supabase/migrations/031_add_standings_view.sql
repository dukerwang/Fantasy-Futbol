-- Create a view to automatically calculate league standings and ranks.
-- Points: Win = 3, Draw = 1, Loss = 0
-- Draw Rule: Gap <= 10 points
-- Tiebreakers: League Points -> Goal Difference -> Points For
...
WITH matchup_results AS (
  ...
  SELECT
    ...
    COALESCE(score_a, 0) AS pf,
    COALESCE(score_b, 0) AS pa,
    CASE 
      WHEN ABS(COALESCE(score_a, 0) - COALESCE(score_b, 0)) <= 10 THEN 1 -- Draw
      WHEN COALESCE(score_a, 0) > COALESCE(score_b, 0) THEN 3 -- Win
      ELSE 0 -- Loss
    END AS points,
  ...
  SELECT
    ...
    COALESCE(score_b, 0) AS pf,
    COALESCE(score_a, 0) AS pa,
    CASE 
      WHEN ABS(COALESCE(score_a, 0) - COALESCE(score_b, 0)) <= 10 THEN 1 -- Draw
      WHEN COALESCE(score_b, 0) > COALESCE(score_a, 0) THEN 3 -- Win
      ELSE 0 -- Loss
    END AS points,
    CASE WHEN ABS(COALESCE(score_a, 0) - COALESCE(score_b, 0)) <= 10 THEN 1 ELSE 0 END AS draws,
...
SELECT
  ...
  RANK() OVER (
    ...
    ORDER BY COALESCE(tt.league_points, 0) DESC, COALESCE(tt.goal_difference, 0) DESC, COALESCE(tt.points_for, 0) DESC
  ) AS rank
FROM teams t
