-- ============================================================
-- Migration 084: Fix execute_loan_acceptance_rpc to accept
--                accepted_deferred loans from matchupProcessor
-- ============================================================
--
-- Bug: The RPC blocked on status <> 'pending', but loans deferred during a
-- live GW have status 'accepted_deferred'. At gameweek end, matchupProcessor
-- calls this RPC on those loans and they were all being rejected (the
-- fallback on RPC failure in matchupProcessor.ts sets the loan to
-- 'rejected'), so a loan accepted mid-gameweek could never complete. Trades
-- got the analogous fix in migration 058; this mirrors it for loans.
