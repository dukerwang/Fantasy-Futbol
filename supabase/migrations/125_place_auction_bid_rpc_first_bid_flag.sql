-- Gaffa — Migration 125: surface whether a bid was the first ever placed on a player
--
-- place_auction_bid_rpc already reads v_highest_team_id (step 4) before doing
-- anything else — the highest PENDING bidder that existed the instant before
-- this call. It's NULL only when nobody had a live bid on this player yet, in
-- either free-agent or listing form. That's exactly "first bid ever", distinct
-- from the two other cases that also leave outbid_team_id NULL in the existing
-- response: raising your own standing bid uncontested, and Buy Now.
--
-- Added so the route can fire a "bidding is open" notification to the rest of
-- the league on a genuine opener, without also firing it every time someone
-- raises their own solo bid (v_highest_team_id = p_team_id, is_first_bid stays
-- false) or wins outright via Buy Now (route already gates that case off
-- separately). No other behavior changes — same body as migration 119.
...
BEGIN
  ...
  -- 4. Current highest bidder, read before we insert. NULL here means nobody
  --    had a live bid on this player before this call — the signal the route
  --    uses (as is_first_bid below) to fire a "bidding is open" notice instead
  --    of an outbid one.
  ...
  RETURN jsonb_build_object(
    ...
    -- True only when NO team had a live bid on this player before this call —
    -- distinct from an uncontested self-raise, which leaves outbid_team_id
    -- NULL too but is not a new opener.
    'is_first_bid', (v_highest_team_id IS NULL)
  );
END;
$$;
