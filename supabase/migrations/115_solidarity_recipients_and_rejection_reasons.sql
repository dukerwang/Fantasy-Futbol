-- Gaffa — Migration 115: rejection reasons (retroactive) + solidarity recipients
--
-- This DB already carries a live, unrecorded change — `resolve_single_player_auction_rpc`
-- returns a `rejected_claims` array on the no-winner branch, with a per-candidate
-- reason (`insufficient_budget` / `roster_full`), tagged "NEW (115)" in its own
-- comments. It was applied directly (dashboard SQL editor or a session that
-- skipped writing a migration file) and never landed in supabase/migrations —
-- exactly the drift CLAUDE.md warns about. This file captures that change
-- retroactively (CREATE OR REPLACE is idempotent against what's already live)
-- so git and the database agree, and adds the actual reason for this
-- migration: notification support for Scout's Fee and solidarity payments.
--
-- Notifications for those two payments (093/096) don't exist today. A club
-- earning a Scout's Fee or a solidarity credit has no way to find out short of
-- reading their transaction history — the league-wide "Auction Won" email only
-- covers the winner and the bid. `notifyAuctionResolution` already receives
-- `scout_team_id` / `scout_amount`, but not the scout's user_id or team name,
-- and has no visibility into WHICH teams received the solidarity split at all
-- (only the per-club amount and a count). This adds:
--   - `scout_user_id` / `scout_team_name` alongside the existing `scout_team_id`
--   - `solidarity_recipients`: a JSONB array of {team_id, team_name, user_id}
--     for every club that was actually paid, built during the same loop that
--     pays them (so it can never disagree with the transactions it just wrote)
...
DECLARE
  ...
  -- NEW (115 notifications): every club actually paid a solidarity share,
  -- built alongside the payment loop below.
  ...
  -- with no winner at all. (Retroactively captured — see file header.)
...
BEGIN
  ...
  IF v_winner_team_id IS NOT NULL THEN
    ...
    v_result := jsonb_build_object(
      ...
      -- NEW (notifications): who to notify and what to call them, without a
      -- second lookup keyed only on initiator_team_name (a display string).
      ...
      -- NEW (notifications): who was actually paid, built during the payment
      -- loop above.
