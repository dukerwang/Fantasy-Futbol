...
The repo root also holds debug/scraper leftovers (`test-*.js`, `parse-*.js`, `fix_page.js`, `*.json` scrape dumps). Most are historical — don't treat them as reference implementations. **`playwright-sofifa.js` is the exception: it's live, actively-used tooling**, not a leftover — see "Local scraper tooling" below before assuming it's dead code or writing a replacement.
