...
`vercel.json` runs a stats sync at **08:15 UTC** specifically to land the post-lockdown pass. Until it does, matchup cards read "Provisional" rather than "Final" (`src/lib/scoring/gameweekState.ts`).

Note `finished_provisional` lives on FPL **fixtures**, not events. Code that read it off an event was always testing `undefined`; don't reintroduce it.
