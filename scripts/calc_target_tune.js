...
const FLEX_CONFIG = {
  ...
  RW: { flex: 0.10, components: ['threat', 'goal_involvement', 'finishing'] },
...
main().catch(err => {
  ...
  const targets = ["Semenyo", "Foden", "Wirtz", "B.Fernandes"];
  targets.forEach(t => {
    const player = Array.from(playerTotals.values()).find(p => p.name === t);
    if (player) {
      console.log("\n--- " + player.name + " (" + player.position + ") ---");
      console.log("Goals:", player.goals, "Assists:", player.assists);
      // Average out their components across all their played GWs
      let avgThreat = 0, avgCreativity = 0, avgInfluence = 0, avgDefensive = 0, avgImpact = 0;
      let count = 0;
      // We would need access to the raw component scores, but they aren't stored in playerTotals.
      // However we know their overall average points.
      console.log("Avg Pts:", (player.totalPoints / player.gamesPlayed).toFixed(2));
    }
  });
  process.exit(1);
});
