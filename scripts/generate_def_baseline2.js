...
const POSITION_WEIGHTS = {
  ...
  AM: { match_impact: 0.10, influence: 0.10, creativity: 0.20, threat: 0.20, defensive: 0.00, goal_involvement: 0.15, finishing: 0.05, save_score: 0.00 },
...
function computeMatchRating(stats, position, refStats) {
  ...
  // We want to track the defensive raw values
  if (!global.defStatsTracker) global.defStatsTracker = {};
  if (!global.defStatsTracker[position]) global.defStatsTracker[position] = [];
  // Only track for players who played >= 60 minutes
  if (stats.minutes_played >= 60) {
    global.defStatsTracker[position].push(defActionsRaw + csBonus + xgcOutperf - gcPenalty);
  }
