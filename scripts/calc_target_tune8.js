...
const FLEX_CONFIG = {
  GK: { flex: 0.20, components: ['match_impact', 'defensive'] },
  ...
  AM: { flex: 0.25, components: ['creativity', 'threat', 'influence'] },
  LW: { flex: 0.15, components: ['match_impact', 'goal_involvement', 'threat'] },
  RW: { flex: 0.15, components: ['creativity', 'goal_involvement', 'match_impact'] },
  ST: { flex: 0.25, components: ['threat', 'goal_involvement', 'finishing'] },
};

//                                                                                                                        Σ = 1.00
const POSITION_WEIGHTS = {
  GK: { match_impact: 0.35, influence: 0.10, creativity: 0.05, threat: 0.00, defensive: 0.25, goal_involvement: 0.00, finishing: 0.00, save_score: 0.05 },
  CB: { match_impact: 0.35, influence: 0.10, creativity: 0.05, threat: 0.00, defensive: 0.00, goal_involvement: 0.20, finishing: 0.05, save_score: 0.00 },
  LB: { match_impact: 0.25, influence: 0.10, creativity: 0.10, threat: 0.05, defensive: 0.15, goal_involvement: 0.10, finishing: 0.00, save_score: 0.00 },
  RB: { match_impact: 0.20, influence: 0.10, creativity: 0.20, threat: 0.00, defensive: 0.05, goal_involvement: 0.20, finishing: 0.00, save_score: 0.00 },
  DM: { match_impact: 0.30, influence: 0.15, creativity: 0.00, threat: 0.00, defensive: 0.20, goal_involvement: 0.10, finishing: 0.00, save_score: 0.00 },
  CM: { match_impact: 0.10, influence: 0.15, creativity: 0.15, threat: 0.10, defensive: 0.15, goal_involvement: 0.10, finishing: 0.00, save_score: 0.00 },
  LM: { match_impact: 0.20, influence: 0.05, creativity: 0.00, threat: 0.20, defensive: 0.00, goal_involvement: 0.30, finishing: 0.10, save_score: 0.00 },
  RM: { match_impact: 0.15, influence: 0.05, creativity: 0.15, threat: 0.10, defensive: 0.00, goal_involvement: 0.30, finishing: 0.10, save_score: 0.00 },
  AM: { match_impact: 0.00, influence: 0.15, creativity: 0.25, threat: 0.20, defensive: 0.00, goal_involvement: 0.15, finishing: 0.00, save_score: 0.00 }, // Bruno tune
  LW: { match_impact: 0.20, influence: 0.05, creativity: 0.00, threat: 0.20, defensive: 0.00, goal_involvement: 0.30, finishing: 0.10, save_score: 0.00 },
  RW: { match_impact: 0.15, influence: 0.05, creativity: 0.15, threat: 0.10, defensive: 0.00, goal_involvement: 0.30, finishing: 0.10, save_score: 0.00 },
  ST: { match_impact: 0.15, influence: 0.05, creativity: 0.15, threat: 0.15, defensive: 0.00, goal_involvement: 0.15, finishing: 0.10, save_score: 0.00 }, // Haaland buff
};
...
async function main() {
  ...
  for (const gw of finishedGWs) {
    ...
    for (const el of elements) {
      ...
      const { rating, fantasyPoints, scores } = computeMatchRating(stats, granularPos, refStats);
      ...
      if (!playerTotals.has(el.id)) {
        playerTotals.set(el.id, {
          ...
          bestGW: 0,
          gwBreakdown: [],
          compSums: { match_impact: 0, influence: 0, creativity: 0, threat: 0, defensive: 0, goal_involvement: 0, finishing: 0, save_score: 0 }
        });
      }
      ...
      acc.compSums.match_impact += scores.match_impact;
      acc.compSums.influence += scores.influence;
      acc.compSums.creativity += scores.creativity;
      acc.compSums.threat += scores.threat;
      acc.compSums.defensive += scores.defensive;
      acc.compSums.goal_involvement += scores.goal_involvement;
      acc.compSums.finishing += scores.finishing;
      acc.compSums.save_score += scores.save_score;
  ...
  // ── Diagnostics ──────────────────────────────────────────────────────────
  console.log('\n── Target Player Diagnostics (Averages) ────────────────────────');
  const targetNames = [
    'Donnarumma', 'Raya', 'Roefs', 'Sánchez', 'Pickford',
    'Gabriel', 'Saliba', 'Guéhi', 'Virgil', 'Rúben', 'Gvardiol', 'Ballard', 'Thiaw', 'Konsa', 'Alderete',
    'Cucurella', 'Calafiori', 'Hall', "O'Reilly", 'Robinson',
    'James', 'J.Timber', 'Matheus N.', 'Mukiele', 'Kayode',
    'Caicedo', 'Anderson', 'Zubimendi', 'Casemiro', 'Wharton', 'Kamara', 'Tonali',
    'Rice', 'Bruno G.', 'Enzo', 'Szoboszlai', 'Xhaka', 'Onana',
    'B.Fernandes', 'Rogers', 'Buendía', 'Wirtz', 'Foden', 'Palmer',
    'Cherki', 'Mbeumo', 'Wilson', 'Bowen', 'Saka', 'M.Salah',
    'Semenyo', 'Doku', 'Grealish',
    'Haaland', 'João Pedro', 'Ekitiké', 'Thiago', 'Calvert-Lewin'
  ];

  for (const p of sorted) {
    if (targetNames.includes(p.name)) {
      const avgPts = (p.totalPoints / p.gamesPlayed).toFixed(1);
      const avgMI = (p.compSums.match_impact / p.gamesPlayed).toFixed(2);
      const avgInf = (p.compSums.influence / p.gamesPlayed).toFixed(2);
      const avgCre = (p.compSums.creativity / p.gamesPlayed).toFixed(2);
      const avgThr = (p.compSums.threat / p.gamesPlayed).toFixed(2);
      const avgDef = (p.compSums.defensive / p.gamesPlayed).toFixed(2);
      const avgGI = (p.compSums.goal_involvement / p.gamesPlayed).toFixed(2);
      const avgFin = (p.compSums.finishing / p.gamesPlayed).toFixed(2);
      const avgSav = (p.compSums.save_score / p.gamesPlayed).toFixed(2);

      const overallRank = sorted.indexOf(p) + 1;

      console.log(`[${p.position.padEnd(2)}] ${p.name.padEnd(16)} (Rank ${overallRank.toString().padStart(3)}) | Pts: ${avgPts} | MI:${avgMI} Inf:${avgInf} Cre:${avgCre} Thr:${avgThr} Def:${avgDef} GI:${avgGI} Fin:${avgFin} Sav:${avgSav}`);
    }
  }
